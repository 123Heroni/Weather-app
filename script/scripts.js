...
updateTime(); // Call once immediately so it doesn't wait 1 sec to start
...
class WeatherApp {
    ...
    async handleSearch(query) {
        ...
        try {
            const response = await fetch(
                `https://wft-geo-db.p.rapidapi.com/v1/geo/cities?namePrefix=${query}&countryIds=NG&limit=10`, {
                    method: 'GET',
                    headers: {
                        'X-RapidAPI-Key': 'da0aa2711cmshf7623429e69c18fp185601jsnb60be6965d48',
    ...
    showSuggestions(cities) {
        this.suggestions.innerHTML = cities.map(city =>
            `<div class="suggestion-item" onclick="weatherApp.selectCity('${city.name}')">
